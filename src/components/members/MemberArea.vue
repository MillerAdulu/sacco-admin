<template>
  <v-app id="inspire">
    <v-navigation-drawer fixed v-model="drawer" app>
      <v-img v-if="loggedInUser.passportPhoto" :src="loggedInUser.passportPhoto" contain/>
      <v-list dense class="pt-0">
        <v-list-tile :to="{name: `MemberDashboard`}" append>
          <v-list-tile-action>
        ...
        <v-list-tile :to="{name: `DepositProfile`}" append>
          <v-list-tile-action>
        ...
        <v-list-tile :to="{name: `ShareDepositProfile`}" append>
          <v-list-tile-action>
            <v-icon>monetization_on</v-icon>
          </v-list-tile-action>
          <v-list-tile-content>
        ...
        <v-list-tile :to="{name: `LoanProfile`}" append>
          <v-list-tile-action>
        ...
        <v-list-tile :to="{name: `AddressProfile`}" append>
          <v-list-tile-action>
            <v-icon>place</v-icon>
          </v-list-tile-action>
          <v-list-tile-content>
        ...
        <v-list-tile :to="{name: `PaymentMethodsProfile`}" append>
          <v-list-tile-action>
    ...
    <v-toolbar color="primary" dark fixed app>
      ...
      <v-dialog v-model="dialog" width="500">
        <v-btn slot="activator" color="button" dark>Add Deposit</v-btn>

        <v-card>
          <v-card-title class="headline grey lighten-2" primary-title>Add Deposit</v-card-title>

          <v-card-text>This will allow you to add a deposit via mpesa
            <br>
          ...
          <v-card-actions>
            ...
            <v-btn color="button darken-1" flat :loading="depositing" @click="addDeposit">Deposit</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-toolbar>
